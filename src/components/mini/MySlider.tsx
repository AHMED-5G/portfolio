...
import useUtils, {
  osRtl,
} from "react-native-range-slider-expo/src/components/utils";
import Svg, { Path } from "react-native-svg";

const SMALL_SIZE = 24;
const MEDIUM_SIZE = 34;
const LARGE_SIZE = 44;
interface MySliderInterface {
  min: number;
  max: number;
...
export const MySlider = ({
  ...
  >("row");
  const [svgOffset, setSvgOffset] = useState<object>({
    left: (knobSize - 40) / 2,
  });
  ...
  useEffect(() => {
    ...
    setSvgOffset(
      osRtl ? { right: (knobSize - 40) / 2 } : { left: (knobSize - 40) / 2 }
    );
  }, [knobSize]);
  ...
  const setValueStatic = (
    newOffset: number,
    knobSize: number,
    stepInPixels: number
  ) => {
    newOffset =
      Math.round((newOffset + knobSize / 2) / stepInPixels) * stepInPixels -
      knobSize / 2;
